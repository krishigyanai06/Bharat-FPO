...
function Procurement() {
  ...
  const handlePurchaseSubmit = async (e) => {
    ...
    try {
      if (editPurchaseId) {
        await dispatch(updateOrder({ id: editPurchaseId, data: payload })).unwrap();
  ...
  const selectedFarmerObj = farmers.find(f => f._id === purchaseForm.farmer);
  const selectedFarmerRole = selectedFarmerObj?.role || "Farmer";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {activeTab === "purchases" && (
          ...
          >
            <Plus size={14} /> Record Crop Purchase
          </button>
        )}
        {activeTab === "stock" && (
          <button
      ...
      {/* ── CREATE / EDIT PROCUREMENT PO MODAL ── */}
      {showPurchaseModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in">
          <form onSubmit={handlePurchaseSubmit} className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl text-xs font-semibold text-gray-700 flex flex-col max-h-[92vh]">
            ...
            <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/70 rounded-t-3xl">
              <div>
                <h2 className="text-base font-black text-gray-800 tracking-tight">
                  {editPurchaseId ? "Edit Procurement" : "Record Crop Procurement"}
                </h2>
                {editPurchaseId && (
                  <p className="text-[10px] font-bold text-gray-400 mt-0.5">
                    Purchase ID: <span className="font-mono text-brand-700 font-extrabold">{purchaseForm.purchaseId || "—"}</span>
                  </p>
                )}
              </div>
              <button
                ...
                className="w-8 h-8 rounded-full bg-white border border-gray-150 flex items-center justify-center text-gray-400 hover:text-gray-650 hover:shadow-sm transition cursor-pointer active:scale-95"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div className="bg-brand-50/30 border border-brand-100/50 p-4.5 rounded-2xl space-y-3">
                <h3 className="text-[10px] font-extrabold text-brand-800 uppercase tracking-widest flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-3.5 items-end">
                  ...
                  {purchaseForm.farmer && (
                    <div className="px-3.5 py-2.5 bg-white border border-gray-100 rounded-xl flex items-center justify-between shadow-xs">
                      <div>
                        ...
                        <div className="col-span-1.5 flex items-center justify-end gap-1.5">
                          ...
                          {purchaseForm.crops.length > 1 && (
                            <button
                              ...
                              className="text-red-500 hover:text-red-700 p-0.5 hover:bg-red-50 rounded transition cursor-pointer"
                              title="Delete Row"
                            >
                              <MinusCircle size={13} />
                            </button>
              ...
              <div className="border border-gray-150 p-5 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-extrabold text-gray-450 uppercase tracking-wider mb-1">
                      Procurement Date *
                    </label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-[10px] font-extrabold text-gray-455 uppercase tracking-wider mb-1">
                      Procurement Center
                    </label>
                    <input
                      type="text"
              ...
              {/* Financial Section */}
              <div className="bg-gray-50 border border-gray-150 p-5 rounded-2xl flex flex-col space-y-2.5 max-w-sm ml-auto">
            ...
            {/* Footer Buttons */}
            <div className="px-6 py-4.5 border-t border-gray-100 flex gap-3 bg-gray-50/50 rounded-b-3xl">
              <button
                type="button"
                onClick={() => { setShowPurchaseModal(false); resetPurchaseForm(); }}
                className="flex-1 py-3 border border-gray-250 bg-white rounded-xl hover:bg-gray-50 transition font-black text-gray-600 text-xs cursor-pointer active:scale-98"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 py-3 bg-brand-700 hover:bg-brand-850 text-white rounded-xl transition flex items-center justify-center gap-1.5 font-black text-xs cursor-pointer active:scale-98 shadow-sm"
              >
                Save Changes
              </button>
            </div>
